  ...
  <div>
    <div class="toolbar">
      ...
      <div style="display:flex;gap:12px">
        <el-input v-model="keyword" placeholder="搜索用户名/姓名/学号" clearable style="width:240px" @clear="fetchList" @keyup.enter="fetchList" />
    ...
    <el-table :data="list.records" border stripe v-loading="loading">
      ...
      <el-table-column label="角色">
        <template #default="{ row }">
          <el-tag :type="row.role === 0 ? 'danger' : 'success'">{{ row.role === 0 ? '管理员' : '学生' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="状态" width="100">
        <template #default="{ row }">
          <el-tag v-if="row.lockUntil && new Date(row.lockUntil) > new Date()" type="danger" size="small">已锁定</el-tag>
...
const formRef = ref()
const form = reactive({
  username: '', password: '', realName: '', role: 1, studentNo: '', phone: '', gender: 1
})
const formRules = {
...
const resetForm = () => {
  Object.assign(form, { username: '', password: '', realName: '', role: 1, studentNo: '', phone: '', gender: 1 })
  isEdit.value = false
  editId.value = null
...
const handleResetPwd = async () => {
  ...
  try {
    await resetPassword(resetUserId.value, { newPassword: resetForm.newPassword })

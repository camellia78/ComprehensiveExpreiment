<template>
  <div>
    ...
    <!-- 重置密码弹窗 -->
    <el-dialog title="重置密码" v-model="resetVisible" width="400px" @closed="resetForm.value?.resetFields()">
      <el-form :model="resetForm" :rules="resetRules" ref="resetFormRef" label-width="80px">
        <el-form-item label="新密码" prop="newPassword">
          <el-input v-model="resetForm.newPassword" type="password" placeholder="请输入新密码(6-20位)" show-password maxlength="20" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const showResetPwd = (row) => { resetUserId.value = row.id; resetForm.newPassword = ''; resetVisible.value = true }
const handleResetPwd = async () => {
  const valid = await resetFormRef.value.validate().catch(() => false)
  if (!valid) return
  resetting.value = true
  try { await resetPassword(resetUserId.value, { newPassword: resetForm.newPassword }); ElMessage.success('密码重置成功'); resetVisible.value = false } catch (_) {}

  ...
  <div>
    ...
    <el-form :model="form" :rules="rules" ref="formRef" label-width="100px" style="max-width:600px">
      ...
      <el-form-item label="空闲床位" prop="bedId">
        <el-select v-model="form.bedId" placeholder="选择床位" :disabled="!form.buildingId"><el-option v-for="bed in freeBeds" :key="bed.id" :label="getRoomNo(bed.roomId) + ' - 床位' + bed.bedNo" :value="bed.id" /></el-select>
      </el-form-item>
    ...
    <el-table :data="checkins" border stripe>
      ...
      <el-table-column label="床位" width="80" prop="bedId" />
...
const buildings = ref([]); const students = ref([]); const freeBeds = ref([]); const checkins = ref([]); const rooms = ref([])
const formRef = ref()
const form = reactive({ studentId: null, buildingId: null, bedId: null, roomId: null })
const rules = { studentId: [{ required: true }], buildingId: [{ required: true }], bedId: [{ required: true }] }
...
const onBuildingChange = async () => { form.bedId = null; form.roomId = null; freeBeds.value = await getFreeBeds(form.buildingId) }

const handleSubmit = async () => {
  if (!(await formRef.value.validate().catch(() => false))) return
  const bed = freeBeds.value.find(b => b.id === form.bedId)
  form.roomId = bed.roomId
